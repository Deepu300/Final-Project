...
async function handleClick(event) {
    ...
    console.log("Gesture result from webcam:", results);
...
async function predictWebcam() {
    ...
    const drawingUtils = new DrawingUtils(canvasCtx); // ✅ Defined here
    ...
    if (results.landmarks) {
      ...
      if (results.gestures.length > 0) {
        ...
        // If the gesture is "Thumb_Up", spawn hearts
        if (categoryName === "Thumb_Up") {
          hearts.push(new Heart(width / 2, height - 50)); // center-bottom
        }
      }
...
class Heart {
  ...
  displayHeart() {
    ...
    bezierVertex(this.x - this.size, this.y - this.size,
                 this.x - this.size * 1.5, this.y + this.size / 2,
                 this.x, this.y + this.size * 1.5);
    bezierVertex(this.x + this.size * 1.5, this.y + this.size / 2,
...
function setup() {
  ...
  canvas.parent(webcamContainer); // 🔥 Attach canvas inside webcamContainer
